...
public class MLumpSumRentConfig extends X_TF_LumpSumRent_Config {
	...
	public static MLumpSumRentConfig getFreightConfig(Properties ctx,int AD_Org_ID,int Vendor_ID, int C_BPartner_ID, int M_Product_ID, int TF_Destination_ID, 
			int TF_VehicleType_ID,BigDecimal Distance, String trxName, String isTransporter) {		
		String Where;
		...
		boolean ENABLE_TRANSPORTER_RENT_CONFIG = MSysConfig.getBooleanValue("ENABLE_TRANSPORTER_RENT_CONFIG", false, Env.getAD_Client_ID(ctx));
		...
		if(ENABLE_TRANSPORTER_RENT_CONFIG) {
			Where = Where + " AND IsTransporter = '"+ isTransporter +"'";
		}
		MLumpSumRentConfig lumpDistConfig=new Query(ctx, Table_Name, Where, trxName)
				.setClient_ID()
				.setOnlyActiveRecords(true)
		...
		//6 -- Vehicle Type, Destination
		Where=" AD_Org_ID=? AND Vendor_ID IS NULL AND  C_BPartner_ID IS NULL AND TF_VehicleType_ID=? AND COALESCE(M_Product_ID,0) IN (?,0) AND "
				+ "(COALESCE(TF_Destination_ID,0) = ? OR (C_UOM_ID IN (?,?) AND TF_Destination_ID IS NULL))";
		...
		lumpDistConfig=new Query(ctx, Table_Name, Where, trxName)
				.setClient_ID()
				.setOnlyActiveRecords(true)
				.setParameters(AD_Org_ID, TF_VehicleType_ID, M_Product_ID, TF_Destination_ID, KM_UOM_ID, MT_KM_UOM_ID)
				.setOrderBy("COALESCE(TF_Destination_ID,0) DESC")
				.first();
		if(lumpDistConfig != null)
			return lumpDistConfig;
	...
	public static MLumpSumRentConfig getFreightPrice(Properties ctx,int AD_Org_ID,int Vendor_ID, int C_BPartner_ID, int M_Product_ID, int TF_Destination_ID, 
			int TF_VehicleType_ID,BigDecimal Distance, int C_UOM_ID, String trxName, String isTransporter) {		
		String Where;
		int KM_UOM_ID = MSysConfig.getIntValue("KM_UOM", 1000071, Env.getAD_Client_ID(ctx));
		int MT_KM_UOM_ID = MSysConfig.getIntValue("MT_KM_UOM", 1000071, Env.getAD_Client_ID(ctx));
		boolean ENABLE_TRANSPORTER_RENT_CONFIG = MSysConfig.getBooleanValue("ENABLE_TRANSPORTER_RENT_CONFIG", false, Env.getAD_Client_ID(ctx));
	...
	protected boolean beforeSave(boolean newRecord) {
		...
		if(isTransporter() && ENABLE_TRANSPORTER_RENT_CONFIG) {
			String whereClause = "AD_Org_ID = ? AND TF_VehicleType_ID = ? AND IsTransporter = ? AND COALESCE(TF_Destination_ID,0) = ? AND "
					+ " COALESCE(Vendor_ID,0) = ? AND COALESCE(C_BPartner_ID,0) = ? AND COALESCE(M_Product_ID,0) = ?";
			
			List<MLumpSumRentConfig> rentconfigs = new Query(getCtx(), MLumpSumRentConfig.Table_Name, whereClause, get_TrxName())
													.setClient_ID()
													.setParameters(getAD_Org_ID(), getTF_VehicleType_ID(), "Y", getTF_Destination_ID(), 
																   getVendor_ID(), getC_BPartner_ID(), getM_Product_ID()).list();
			if(rentconfigs.size() > 0)
				throw new AdempiereException("Rent Config already exists for the transporter!");
		}
