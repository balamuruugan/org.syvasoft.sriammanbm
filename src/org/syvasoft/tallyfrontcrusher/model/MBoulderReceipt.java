...
public class MBoulderReceipt extends X_TF_Boulder_Receipt {
		...
		if(newRecord || is_ValueChanged(COLUMNNAME_C_Project_ID)) {
			if(getC_Project_ID() > 0) {
				...
				if(getTF_Boulder_Wastage_ID() > 0) {
					if(proj.getJobWork_Product_ID() != proj.getJobWorkWOTrans_Product_ID()) {
						throw new AdempiereException("Error: With Transport & Without Transport not implemented for Boulder Wastage Approach!");
					}
	...
	public void createFromBoulderWastage(MBoulderWastageHdr wastage, String sendTo) {
	/*	setAD_Org_ID(wastage.getAD_Org_ID());
		setDateReceipt(wastage.getDateAcct());
		setDateAcct(wastage.getDateAcct());
		
		TF_MProject proj = new TF_MProject(getCtx(), wastage.getQuarry_Project_ID(), get_TrxName());	
		
		if(proj != null && proj.getC_Project_ID() > 0) {
			setC_Project_ID(wastage.getQuarry_Project_ID());
			setSubcontractor_ID(wastage.getSubcontractor_ID());
			setTF_Quarry_ID(proj.getTF_Quarry_ID());									
			setC_UOM_ID(proj.getC_UOM_ID());
		}
		
		setM_Product_ID(wastage.getM_Product_ID());		
		
		if(sendTo.equals(MWeighmentEntry.TF_SEND_TO_Production))
			setQtyReceived(wastage.getProduction_QtyWastage().negate());
		else
			setQtyReceived(wastage.getStock_QtyWastage().negate());
		
		setM_Warehouse_ID(wastage.getM_Warehouse_ID());
		setTF_Send_To(sendTo);
		setDescription(wastage.getDescription());
		setDocStatus(DOCSTATUS_Drafted);
		setDocAction(DOCACTION_Complete);
		setTF_Boulder_Wastage_ID(wastage.getTF_Boulder_Wastage_ID());		*/
	}
	...
	public void reverseIt() {
		...
		/*
		else if(getM_Product().isStocked()) {
			MWarehouse warehouse = MWarehouse.get(getCtx(), getM_Warehouse_ID());
			int defaultLocatorID = warehouse.getDefaultLocator().getM_Locator_ID();
			String m_processMsg; 
			//Update Storage for the received Product from the Joborder.
	...
	private void createInternalUseInventory() {
		...
		if(getTF_Boulder_Wastage_ID() > 0)
			inv.addDescription(" Boulder Wastage No: " + getTF_Boulder_Wastage().getDocumentNo());
	...
	public void CreateSubcontract2ServiceReceipt(int C_BPartnerSubcon2_ID, int M_ProductSubcon2_ID, BigDecimal priceSubcon2) {
		...
		if(getC_Project_ID() > 0) {
			TF_MProject proj = new TF_MProject(getCtx(), getC_Project_ID(), get_TrxName());
			ioLine.setC_Tax_ID(proj.getC_Tax_ID());
		}
		ioLine.set_ValueOfColumn("DocStatus", "CO");
		ioLine.saveEx(get_TrxName());
	...
	public void reverseServiceReceipts() {
		...
		whereClause = "TF_Boulder_Wastage_ID = ? AND MovementType = ? AND DocStatus = 'CO'";
		list = new Query(getCtx(), TF_MInOut.Table_Name, whereClause, get_TrxName())
				.setClient_ID()
				.setParameters(getTF_Boulder_Wastage_ID(), MInOut.MOVEMENTTYPE_VendorReceipts)
				.list();
		for(TF_MInOut io : list) {
			if(io.getDocStatus().equals(DOCSTATUS_Completed))
				io.reverseCorrectIt();
	...
	public static BigDecimal getReceiptQty(int C_BPartner_ID, int M_Warehouse_ID, Timestamp DateAcct, String SendTo, String trxName) {
		// Consolidated invoice line will be created only for the new Products which are not in Invoice Lines already.
		String sql = "SELECT sum(coalesce(qtyreceived,0)) FROM tf_boulder_receipt WHERE " + 
				" docstatus IN ('CO','CL') AND subcontractor_id = ? AND m_warehouse_id = ? AND trunc(dateacct) = ?";
