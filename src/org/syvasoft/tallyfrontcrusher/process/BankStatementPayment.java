...
public class BankStatementPayment extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		...
		if (Table_ID == X_I_BankStatement.Table_ID)
			...
			return createPayment (new TF_MBankStatementLine(getCtx(), Record_ID, get_TrxName()));
...
//				&& line.getC_Invoice_ID() > 0
	 ...
	 *  @throws Exception if not successful
	 */
	private String createPayment (TF_MBankStatementLine bsl) throws Exception
	{
		...
		MPayment payment = createPayment (bsl.getC_Invoice_ID(), bsl.getC_BPartner_ID(),
			...
			bsl.getDescription(), bsl.getAD_Org_ID(), bsl.getC_ElementValue_ID());
		if (payment == null)
			throw new AdempiereSystemError("Could not create Payment");
		//	update statement
	 ...
	 *	@return payment
	 *
	 *  Do not create payment when the payment reference is there with charge or interest amount
	 *  because these charge amount and interest amount will be posted separately while completing bank statement	 *  
	 *  
	 */
	private MPayment createPayment (int C_Invoice_ID, int C_BPartner_ID, 
		...
		String Description, int AD_Org_ID, int C_ElementValue_ID)	
	{
		boolean isSOTrx = StmtAmt.doubleValue() > 0; 
		...
		payment.setTF_BPartner_ID(C_BPartner_ID);
		...
		payment.setC_ElementValue_ID(C_ElementValue_ID);
		payment.setIsEmployee(C_ElementValue_ID > 0);		
		//
		if (C_Invoice_ID != 0)
		{
