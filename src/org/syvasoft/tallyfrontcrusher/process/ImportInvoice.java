	...
	protected String doIt() throws java.lang.Exception
	{
		...
		*/
		...
		if(!m_validateOnly) {
			...
			StringBuilder innersql = new StringBuilder ("SELECT count(*) cnt FROM I_Invoice ")
			...
			if(errCount == 0) {
				...
				sql = new StringBuilder ("SELECT * FROM I_Invoice ")
					...
					.append(" ORDER BY C_DocType_ID,DocumentNo,C_BPartner_ID, C_BPartner_Location_ID, I_Invoice_ID");
				try
				{
					pstmt = DB.prepareStatement (sql.toString(), get_TrxName());
					rs = pstmt.executeQuery ();
