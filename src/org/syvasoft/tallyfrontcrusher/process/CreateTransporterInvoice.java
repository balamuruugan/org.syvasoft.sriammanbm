	...
	protected String doIt() throws Exception {
		...
		if(automatic_process) {
			whereClause = " DocStatus = 'CO' AND (EXISTS (SELECT m_inout_id FROM m_inout io JOIN tf_weighmententry w ON w.tf_weighmententry_id = io.tf_weighmententry_id WHERE io.issotrx='N'" + 
					" AND io.m_inout_id = m_inoutline.m_inout_id AND io.docstatus <> 'RE' AND w.Status IN ('CO', 'CL'))) ";
			...
			if(M_InoutLine_ID > 0) {
				whereClause = " DocStatus = 'CO' AND M_InOutLine_ID = ?";
		...
		for(TF_MInOutLine ioLine : list) {
			...
			if(ioLine.getQtyEntered().doubleValue() == 0) {
				addLog(0, null, null, "Doc No: " + io.getDocumentNo() + " - Invalid Qty!" , io.get_Table_ID(), io.get_ID());
				
				if(!automatic_process) {
					return "Please set the Qty!";
				}
			}
