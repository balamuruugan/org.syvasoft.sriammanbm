...
public class ImportOrder extends SvrProcess
{
	...
	private boolean 		m_validateOnly = false;
	/**	Document Action					*/
	...
	protected void prepare()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			else if (name.equals("ValidateOnly"))
				m_validateOnly = "Y".equals(para[i].getParameter());
			else
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		if(!m_validateOnly) {
			...
			StringBuilder innersql = new StringBuilder ("SELECT count(*) cnt FROM I_Order ")
					  .append("WHERE I_IsImported='E'");
			
			int errCount = DB.getSQLValue(get_TrxName(), innersql.toString());
					
			if(errCount == 0) {
				//	Go through Order Records w/o
				sql = new StringBuilder ("SELECT * FROM I_Order ")
					  .append("WHERE I_IsImported='N'").append (clientCheck)
					.append(" ORDER BY C_DocType_ID,DocumentNo,C_BPartner_ID, BillTo_ID, C_BPartner_Location_ID, I_Order_ID");
				/*try
				{*/
					pstmt = DB.prepareStatement (sql.toString(), get_TrxName());
					rs = pstmt.executeQuery ();
	...
	*/	
				...
				addLog (0, null, new BigDecimal (errCount), "Error Records");
			}
