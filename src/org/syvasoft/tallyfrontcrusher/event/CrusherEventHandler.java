...
public class CrusherEventHandler extends AbstractEventHandler {
	...
	protected void initialize() {
		...
		registerTableEvent(IEventTopics.DOC_BEFORE_COMPLETE, TF_MInvoice.Table_Name);
		registerTableEvent(IEventTopics.DOC_BEFORE_COMPLETE, TF_MPayment.Table_Name);
	...
	protected void doHandleEvent(Event event) {
		...
		if(event.getTopic().equals(IEventTopics.DOC_BEFORE_COMPLETE)) {
			if(po.get_TableName().equals(MPayment.Table_Name) || po.get_TableName().equals(MInvoice.Table_Name)) {
				TF_MBPartner bp = new TF_MBPartner(po.getCtx(), po.get_ValueAsInt("C_BPartner_ID"), po.get_TrxName());
				if(bp.isCustomer() || bp.isVendor()) {
					bp.setOutstandingBalance((Timestamp) po.get_Value("DateAcct"));					
					bp.saveEx(po.get_TrxName());
		...
		if(po.get_TableName().equals(MPayment.Table_Name)) {
			...
			if(event.getTopic().equals(IEventTopics.PO_BEFORE_NEW)) {
				...
				if(payment.getC_Invoice_ID() > 0 && payment.getReversal_ID() == 0) {
					...
					if(inv.getC_Order_ID() > 0 && payment.getTenderType().equals("X") && (inv.getPaymentRule().equals(MInvoice.PAYMENTRULE_Cash)
							|| inv.getPaymentRule().equals("O") )) {
